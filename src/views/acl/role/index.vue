...
async function getRoleList(page = 1, rolename = '') {
  // 获取角色列表
  currentPage.value = page
  const res = await getAllRoleList(currentPage.value, pageSize.value, rolename)
  if (res.code === 200) roleListData.value = res.data.records
...
function handleUpdate(row: rolesObj) {
  // 编辑角色
...
async function handleDelete(row: rolesObj) {
  // 删除某个角色
  ElMessageBox.confirm(`是否确认删除${row.roleName}?`, '提示', {
...
async function batchDelete() {
  ...
  if (res.code === 200) {
    getRoleList(
      roleListData.value.length > 1 ? currentPage.value : currentPage.value - 1,
    ) // 如果删除后当前页还有数据，就留在当前，否则获取上一页的数据
...
async function submit() {
  ...
  if (res.code === 200) {
    ...
    dialogVisible.value = false
    if (roleParams.value.id) window.location.reload() // 如果修改的是当前登录的用户，则需要刷新页面，返回到登录页
  } else {
...
async function handleRole(row: rolesObj) {
  // 分配角色,打开抽屉，获取全部的角色列表
...
// 第一种写法
function filterSelectRoleId(arr: any) {
  // 过滤出角色列表中以选中的权限id,注意这里要拿旧数组拼接，否则数据丢失
  let res: string[] = []
  arr.forEach((item: any) => {
    if (item.select && item.level === 4)
      // if (item.select && item.level === 4) 这里选第4级，是因为这一级才有子数据
      res.push(item.id)
...
async function assignRole() {
  ...
  const res = await doAssignPermission({
    permissionIdList: [
      ...tree.value.getCheckedKeys(),
      ...tree.value.getHalfCheckedKeys(),
    ],
...
function search() {
  // 根据用户名进行搜索
  if (keyword.value) getRoleList(1, keyword.value)
  keyword.value = ''
}
function reset() {
  // 重置按钮
...
<template>
  <div>
    ...
    <el-card class="mt-5">
      ...
      >
        <el-row :gutter="10">
          <el-col :span="6" :offset="0">角色名称</el-col>
        ...
        <el-row :gutter="10" class="mt-5">
          <el-col :span="6" :offset="0">菜单列表</el-col>
          <el-col :span="18" :offset="0">
