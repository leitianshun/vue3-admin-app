...
function handleUpdate(row: recordsType) { // 编辑,当前后台做了处理，无法修改
...
async function handleDelete(row: recordsType) { // 删除某个用户
  ElMessageBox.confirm(
    ...
    .then(() => {
      delUser(row.id!).then((res) => {
        if (res.code === 200) {
          getUsers(userListData.value.length > 1 ? currentPage.value : currentPage.value - 1) // 如果删除后当前页还有数据，就留在当前，否则获取上一页的数据
