...
function getData() {
  // 这可以写为getData(page = 1){ currentPage.value = page} 当切换size时自动获取第一页的数据
...
function deleteTradmark(row: any) {
  ElMessageBox.confirm(`是否确认删除${row.tmName}?`, '提示', {
    type: 'warning',
    icon: markRaw(Delete),
  }).then(() => {
    delTradmark(row.id)
    ...
    >
      <el-form ref="ruleFormRef" :model="formData" :rules="rules">
        ...
        <el-form-item label="品牌LOGO" label-width="" prop="logoUrl">
          ...
          >
            <img
              v-if="formData.logoUrl && formData.id"
